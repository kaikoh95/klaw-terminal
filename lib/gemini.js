...
/**
 * Analyze market data and technicals with Gemini AI (with optional multi-timeframe data)
 */
export async function analyzeWithGemini(marketData, technicals, multiTimeframe = null) {
  if (!GEMINI_API_KEY) {
...
function buildAnalysisPrompt(marketData, technicals, multiTimeframe = null) {
  ...
  const { movingAverages, rsi, mfi, macd, stochastic, bollingerBands, vwap, obv, obvDivergence, cmf, atr, superTrend, adx, fibonacci, nearestFib, supportResistance, trend, candlestickPatterns, volumeProfile, heikinAshi, ichimoku, volumeDelta } = technicals;
...
• Multi-System Confluence (MTF + Ichimoku + VP POC + Fib + Pattern + HA + VDelta): MAXIMUM SETUP (+7-9)
 ...
 */
export async function batchAnalyze(marketDataMap, technicalsMap, multiTimeframeMap = {}) {
  const results = {};
  ...
  for (const [ticker, marketData] of Object.entries(marketDataMap)) {
    ...
    try {
      const analysis = await analyzeWithGemini(marketData, technicals, multiTimeframe);
