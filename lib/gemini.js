...
function buildAnalysisPrompt(marketData, technicals, multiTimeframe = null) {
  ...
  const { movingAverages, vwma, rsi, mfi, macd, stochastic, bollingerBands, vwap, obv, obvDivergence, cmf, atr, psar, superTrend, adx, elderRay, fibonacci, nearestFib, supportResistance, trend, candlestickPatterns, priceActionPatterns, volumeProfile, heikinAshi, ichimoku, volumeDelta } = technicals;
...
• Williams %R ${technicals.williamsR?.value?.toFixed(1) || 'N/A'} ${technicals.williamsR?.signal?.toUpperCase() || 'N/A'} (${technicals.williamsR?.strength || 'none'}) ${technicals.williamsR?.value >= -20 ? '🔴OB' : technicals.williamsR?.value <= -80 ? '🟢OS' : technicals.williamsR?.value >= -40 ? '↗️' : '↘️'}${technicals.williamsR?.divergence ? ` 🚨${technicals.williamsR.divergence.type.toUpperCase()} DIV (Conf: ${technicals.williamsR.divergence.confidence}/10)` : ''}
...
• Elder-Ray: ${elderRay ? `${elderRay.signal.toUpperCase()} (${elderRay.strength}, Conf: ${elderRay.confidence}/10) | Bull: ${elderRay.bullPower.toFixed(3)} ${elderRay.bullTrend === 'rising' ? '↑' : '↓'} | Bear: ${elderRay.bearPower.toFixed(3)} ${elderRay.bearTrend === 'rising' ? '↑' : '↓'} ${elderRay.bullPower > 0 && elderRay.bearPower > 0 ? '🟢BULLS DOMINANT' : elderRay.bullPower < 0 && elderRay.bearPower < 0 ? '🔴BEARS DOMINANT' : '⚠️MIXED'}${elderRay.divergence ? `\n  🚨 ${elderRay.divergence.type.toUpperCase()} ELDER-RAY DIVERGENCE! ${elderRay.divergence.signal} (Conf: ${elderRay.divergence.confidence}/10)` : ''}` : 'N/A'}
• Parabolic SAR: ${psar ? `$${psar.value.toFixed(2)} ${psar.signal.toUpperCase()} (${psar.strength}) ${psar.trend === 'uptrend' ? '🟢↑' : '🔴↓'} | Trail Stop: $${psar.trailStop.toFixed(2)} | Distance: ${psar.distance.toFixed(2)}% | Momentum: ${psar.consecutiveTrend}p ${psar.recentReversal ? '🔄FRESH SIGNAL!' : ''}` : 'N/A'}
...
• Elder-Ray Divergence (strong, conf ≥8): +1.5 to +2
• Volume Delta Divergence (strong): +1.5 to +2
• OBV Divergence (strong, overrides ADX): +1.5 to +2
• Elder-Ray Bull/Bear Power Extreme (both positive or both negative, very_strong): +1
