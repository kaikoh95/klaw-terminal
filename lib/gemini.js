...
function buildAnalysisPrompt(marketData, technicals) {
  ...
  const { movingAverages, rsi, mfi, macd, stochastic, bollingerBands, vwap, obv, obvDivergence, atr, adx, fibonacci, nearestFib, supportResistance, trend, candlestickPatterns, volumeProfile } = technicals;
...
- MFI(14): ${mfi?.value.toFixed(2) || 'N/A'} ${mfi ? mfi.value > 80 ? '🔴 OVERBOUGHT' : mfi.value < 20 ? '🟢 OVERSOLD' : mfi.value > 60 ? 'Bullish' : mfi.value < 40 ? 'Bearish' : 'Neutral' : ''}
  - Signal: ${mfi ? mfi.signal.toUpperCase() : 'N/A'} ${mfi ? `(${mfi.strength.toUpperCase().replace('_', ' ')})` : ''}
  - Money Flow Ratio: ${mfi ? mfi.moneyFlowRatio.toFixed(2) : 'N/A'}
  - ${mfi ? '💡 Volume-weighted RSI - Strong divergence from RSI can signal volume-backed moves' : ''}
...
### Volume Profile (${volumeProfile ? volumeProfile.lookback : 'N/A'}-period)
${volumeProfile ? `
📊 **Volume-Based Support/Resistance** (Where actual trading occurred)
- **Point of Control (POC)**: $${volumeProfile.poc.toFixed(2)} ${Math.abs(price - volumeProfile.poc) / volumeProfile.poc < 0.005 ? '← CURRENT PRICE (Strong S/R!)' : ''}
  💡 Highest volume price level - acts as a magnet (strong support/resistance)
- **Value Area High (VAH)**: $${volumeProfile.vah.toFixed(2)} ${price >= volumeProfile.vah ? '← Price above value area (BULLISH)' : ''}
- **Value Area Low (VAL)**: $${volumeProfile.val.toFixed(2)} ${price <= volumeProfile.val ? '← Price below value area (BEARISH)' : ''}
  💡 70% of volume occurred between VAL and VAH

**Current Position:** ${volumeProfile.position.toUpperCase().replace('_', ' ')}
${volumeProfile.position === 'above_value_area' ? '  ⚠️ Price trading ABOVE where most volume occurred - potential pull-back to VAH/POC' : ''}
${volumeProfile.position === 'below_value_area' ? '  ⚠️ Price trading BELOW where most volume occurred - potential bounce to VAL/POC' : ''}
${volumeProfile.position === 'at_poc' ? '  🎯 Price AT Point of Control - critical decision zone! Watch for acceptance or rejection' : ''}
${volumeProfile.position === 'in_value_area' ? '  ✅ Price within Value Area - normal trading range, less likely to see big moves unless breakout' : ''}

**Why Volume Profile Matters:**
- POC = Where most traders have positions (high liquidity, strong S/R)
- VAH/VAL = Boundaries of "fair value" - price tends to revert to this range
- Price above VAH = Strong momentum, but beware of mean reversion
- Price below VAL = Weak momentum, watch for oversold bounce
- **Use with price-based S/R for confluence zones = highest probability setups**
...
${candlestickPatterns && candlestickPatterns.primary ? `
🕯️ **${candlestickPatterns.primary.pattern} detected!**
  - Signal: ${candlestickPatterns.primary.signal.toUpperCase()}
  - Strength: ${candlestickPatterns.primary.strength.toUpperCase().replace('_', ' ')}
  - Context: ${trend.toUpperCase().replace('_', ' ')} trend
  ${candlestickPatterns.count > 1 ? `- Additional patterns: ${candlestickPatterns.all.slice(1).map(p => p.pattern).join(', ')}` : ''}
  
**Pattern Interpretation:**
${candlestickPatterns.primary.pattern === 'Morning Star' || candlestickPatterns.primary.pattern === 'Evening Star' ? '⭐ Three-candle reversal - very high probability when confirmed!' : ''}
${candlestickPatterns.primary.pattern === 'Bullish Engulfing' || candlestickPatterns.primary.pattern === 'Bearish Engulfing' ? '🔥 Strong reversal signal - watch for volume confirmation!' : ''}
${candlestickPatterns.primary.pattern === 'Hammer' || candlestickPatterns.primary.pattern === 'Shooting Star' ? '🔨 Single-candle reversal - confirm with next candle close!' : ''}
${candlestickPatterns.primary.pattern === 'Doji' || candlestickPatterns.primary.pattern === 'Dragonfly Doji' || candlestickPatterns.primary.pattern === 'Gravestone Doji' ? '⚖️ Indecision/reversal - important inflection point!' : ''}
${candlestickPatterns.primary.pattern === 'Three White Soldiers' || candlestickPatterns.primary.pattern === 'Three Black Crows' ? '📊 Strong continuation pattern - trend has momentum!' : ''}
${candlestickPatterns.primary.pattern === 'Piercing Line' || candlestickPatterns.primary.pattern === 'Dark Cloud Cover' ? '☁️ Partial reversal - confirm with support/resistance!' : ''}

**Signal Priority:** ${candlestickPatterns.primary.strength === 'very_strong' ? 'HIGH (can override conflicting indicators when combined with volume + support/resistance)' : candlestickPatterns.primary.strength === 'strong' ? 'MODERATE-HIGH (strong confirming signal)' : 'MODERATE (use as confirmation only)'}
...
2. **Momentum Quality** (30% weight)
   ...
   - **MFI confirmation** - When MFI and RSI diverge, favor MFI (volume-backed)
   - Volume confirmation
   - Rate of change
...
3. **Entry/Exit Precision** (20% weight)
   ...
   - **Volume Profile levels (POC, VAH, VAL) - volume-backed S/R**
...
- **Volume Profile Confluence** = Price near POC/VAH/VAL (volume-backed S/R levels)
  - POC = Highest volume price level - acts as magnet, expect reaction (+1.5 confidence)
  - VAH/VAL = Value area boundaries - strong breakout/reversal zones (+1 confidence)
  - POC + Fibonacci level = Double confluence (highest probability, +2 confidence)
  - Price rejection at POC with volume = Very strong reversal signal
...
- **MFI/RSI Divergence** = Volume-backed momentum shift
  - When MFI shows opposite signal from RSI, volume is telling a different story
  - MFI oversold (<20) + RSI neutral/bullish = Strong buy with volume support (+1.5 confidence)
  - MFI overbought (>80) + RSI neutral/bearish = Strong sell with volume confirmation (+1.5 confidence)
  - MFI and RSI both extreme in same direction = Very high conviction (+2 confidence)
- **Candlestick Reversal Patterns** (Morning/Evening Star, Engulfing) = High-probability turning points
  - Very Strong patterns can justify LONG/SHORT when aligned with support/resistance
  - Increases confidence by +1-2 points depending on strength
  - Must confirm with volume and price level confluence
- Fibonacci 61.8% + RSI/MFI oversold/overbought = High probability bounce/rejection
...
- Volume Profile POC + Candlestick pattern + Fibonacci = ULTRA-CONFLUENCE (highest confidence, +3)
